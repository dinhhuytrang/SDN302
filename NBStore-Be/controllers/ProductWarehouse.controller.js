...
const updateProductWarehouse = async (req, res, next) => {
  try {
    ...
    const updatedProductWarehouse = await ProductWarehouse.findByIdAndUpdate(
      productId,
      req.body,
      { new: true }
    );
...
const deleteProductWarehouse = async (req, res, next) => {
  try {
    const productId = req.params.id;
    const deletedProductWarehouse = await ProductWarehouse.findByIdAndDelete(
      productId
    );
...
const checkProductInWarehouse = async (req, res, next) => {
  try {
    ...
    const productInWarehouse = await ProductWarehouse.findOne({
      product: existingProduct._id,
    });
